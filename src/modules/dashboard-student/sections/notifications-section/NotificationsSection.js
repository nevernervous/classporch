...
class NotificationsSection extends React.Component{
	...
	componentWillReceiveProps(nextProps){
		if(nextProps.notifications.length !== this.props.notifications.length){
			const {displayClickCount} = this.state
			this.setState({
				allNotifications:nextProps.notifications, 
				displayNotifications: nextProps.notifications.slice(0,3),
				displayMoreNotifications: nextProps.notifications.slice(0,3*displayClickCount) 
			})
		}
	}
	...
	populateNotificationStrips = (notifications) => {
		const {userId, firstName, lastName, role} = this.props
		return notifications.map( (notification,i) => {
			console.log(notification)
			return <NotificationStrip notification = {notification} 
						userId={userId} firstName={firstName} lastName={lastName} role={role}
			 key = {i} />
		})
	}
